<div wire:poll.1s>
    @if($activeTimer || $pausedTimer)
        ...
        <div class="mx-2 mb-3 rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-white/10 dark:bg-white/5">
            ...
            @if($activeTimer)
                <div class="mt-3 grid grid-cols-[40px_1fr] gap-2"><button wire:click="pause" class="flex size-10 items-center justify-center rounded-lg bg-zinc-200 dark:bg-white/10" aria-label="Pause timer" title="Pause timer"><flux:icon name="pause" class="size-4" /></button><flux:modal.trigger name="sidebar-book-timer"><button class="flex h-10 w-full items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-falqo-600 px-2 text-xs font-semibold text-white"><flux:icon name="check" class="size-3.5" />Book time</button></flux:modal.trigger></div>
            @else
                <div class="mt-3 grid grid-cols-[40px_1fr] gap-2"><button wire:click="resume" class="flex size-10 items-center justify-center rounded-lg bg-zinc-200 dark:bg-white/10" aria-label="Resume timer" title="Resume timer"><flux:icon name="play" class="size-4" /></button><flux:modal.trigger name="sidebar-book-timer"><button class="flex h-10 w-full items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-falqo-600 px-2 text-xs font-semibold text-white"><flux:icon name="check" class="size-3.5" />Book time</button></flux:modal.trigger></div>
            @endif
        </div>
    @endif
