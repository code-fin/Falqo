...
class Workspace extends Component
{
    ...
    {
        $this->validate(['title' => 'required|max:200', 'description' => 'nullable|max:2000', 'customerId' => 'required|integer', 'projectId' => 'nullable|integer', 'assignedUserId' => 'required|integer', 'priority' => 'required|in:low,normal,high,urgent', 'category' => 'required|in:'.implode(',', array_column(TicketCategory::cases(), 'value')), 'ticketStatus' => 'required|in:'.implode(',', array_column(TicketStatus::cases(), 'value')), 'estimatedHours' => 'required|integer|min:0|max:9999', 'estimatedMinutes' => 'required|integer|min:0|max:59']);
        ...
        Ticket::create(['title' => $this->title, 'description' => $this->description ?: null, 'category' => $this->category, 'status' => $this->ticketStatus, 'customer_id' => $this->customerId, 'project_id' => $this->projectId, 'assigned_user_id' => $this->assignedUserId, 'priority' => $this->priority, 'estimated_minutes' => ($this->estimatedHours * 60) + $this->estimatedMinutes, 'time_bookmarked' => $this->timeBookmarked]);
        $this->reset('title', 'description', 'category', 'ticketStatus', 'customerId', 'projectId', 'estimatedHours', 'estimatedMinutes');
